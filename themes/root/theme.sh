...
# TerminuX — Theme definition: ROOT (Kali Linux Root Replica)

THEME_NAME="ROOT"
THEME_DESC="Exact Kali Linux root environment replica (strict and locked)"
...
theme_render_prompt() {
    # ROOT mode strictly replicates the official Kali Linux root prompt
    local p="$4" g="$5"
    local r='\[\e[0m\]'
    local blue='\[\e[1;34m\]'
    local red='\[\e[1;31m\]'
